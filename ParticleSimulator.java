...
// UPDATE:
// I fixed the issue at around 1pm December 5th. It only involved changing _timeOfEvent to _timeEventCreated in my isValid checks in the
// WallCollisionEvent and ParticleCollisionEvent class, and adding "|| event._timeOfEvent < lastTime" in my validity check in the simulate method.
// These were way simpler fixes than I originally thought.
...
public class ParticleSimulator extends JPanel {
	...
	private void simulate(boolean show) {
		...
		while (_events.size() > 0) {
			...
			if (!event.isValid() || event._timeOfEvent < lastTime) { // ADDED "|| event._timeOfEvent < lastTime" AFTER DEADLINE
